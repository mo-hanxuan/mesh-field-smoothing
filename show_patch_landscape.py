...
compared to show_patch_dense.py, here we can show the outerFrames of the densified facets,
and landscape of the facets can be shown by different heights determined by different VF
"""
...
class bodyPatch(object):
                ...
                if averageZ > 0:
                    ...
                    for other in obj.eleNeighbor[ele]:
                        if obj.VF[other] != obj.VF[ele]:
                            ...
                            break
                    if not sameNeighbor:
                        denseOrder = 8  # densify by 4 times
                    else:
                        denseOrder = 1
                    ...
                    for edge in outerFrames:
                        ...
                        edges[xyzs] = list(map(lambda x: float(valueFunc(x)), xyzs))
    ...
    def simple_draw_patch_byOriginal(self, iele, field, minVal=None, maxVal=None, drawArrows=False):
        ...
        facets, edges_tmp = horizonsFacets(obj, iele, inHorizon=obj.inBox)
        for facet in facets:
        ...
        for edge in edges_tmp:
            xyzs = tuple(sorted((
                tuple(obj.nodes[edge[0]]), tuple(obj.nodes[edge[1]])
            )))
            edges[xyzs] = [xyzs[0][2], xyzs[1][2]]
...
if __name__ == '__main__':
    ...
    #     # maxVal=700.,
    # )
    body1.simple_draw_patch_byFit_VF(iele, densify=False, drawArrows=False)
