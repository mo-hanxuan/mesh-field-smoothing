...
class regularDenseNodes(object):
    ...
    def drawDenseNodes(self, region_cen=None, funcVF=None):
        if funcVF == None:
        ...
        mhxOpenGL.showUp(self.__drawDenseNodes__)
        
    
    def __drawDenseNodes__(self):
        denseNodes = self.denseNodes
        obj = self.obj
        ...
        for i in range(len(denseNodes)):
            for j in range(len(denseNodes[i])):
                ...
                red, green, blue = colorBar.getColor(self.denseNodesVF[i][j])
